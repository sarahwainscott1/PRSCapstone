...
namespace PRSCapstone.Controllers {
    ...
    public class RequestLinesController : ControllerBase {
        ...
        //CALCULATE TOTAL
        private async Task<ActionResult> RecalculateRequestTotal(int requestId) {
            var request = await _context.Requests.FindAsync(requestId);
            if (request == null) { throw new Exception("No request {requestId} found"); }
            request.Total = (from rl in _context.RequestLines
                             join p in _context.Products on rl.ProductId equals p.Id
                             where rl.RequestId == requestId
                             select new { RequestTotal = rl.Quantity * p.Price }).Sum(x => x.RequestTotal);
